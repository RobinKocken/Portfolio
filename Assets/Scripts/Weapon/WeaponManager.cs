...
public class WeaponManager : MonoBehaviour
{
    ...
    public List<Weapon> weaponHolder;
    ...
    public void InitializeWeapon(InventoryManager.Holder weaponHolder)
    {
        for(int i = 0; i < weaponHolder.slot.Count; i++)
        {
            if(weaponHolder.slot[i] != null)
            {
                if(weaponHolder.slot[i].itemData != null)
                {
                    GameObject weaponObject = Instantiate(weaponHolder.slot[i].itemData.prefab, transform.position, Quaternion.identity, transform);

                    if(weaponObject.TryGetComponent<Weapon>(out Weapon weapon))
                    {
                        this.weaponHolder.Add(weapon);
                    }
                }
                else if(weaponHolder.slot[i].itemData == null)
                {
                    this.weaponHolder.Add(null);
                }
            }
        }
    }
